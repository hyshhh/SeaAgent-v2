...
class Planner:
    ...
    def decide_tools(
        ...
        calls = self._auto_complete_calls(calls, memory_scope, intent or {})
        ...
        if request_error or not executable:
            ...
            # 先程序补全常见依赖，再决定是否请求模型重规划
            patched = self._auto_complete_calls(calls, memory_scope, intent or {})
            patched_ok, patched_issue = self._calls_are_executable(patched, memory_scope, state if patched else "uncertain")
            if patched and patched_ok:
                calls = patched
                state = "replan" if state == "sufficient" else state
                plan_repair = f"{initial_issue}；已自动补全工具依赖"
            else:
                repair_payload = {
                    **payload,
                    ...
                    "repairHint": "description flow: getTrack -> getFrames -> matchText; use object ref form",
                }
                if on_delta:
                    on_delta("\n[计划校验未通过，正在请求重新规划]\n")
                ...
                repaired_calls = self._auto_complete_calls(repaired_calls, memory_scope, intent or {})
                ...
                if not repair_error and repaired_ok:
                    ...
                    failure = repair_error or repaired_issue or patched_issue or "重规划未生成可执行工具调用"
                    # 最终兜底：描述查询给最小可执行链，避免空计划死循环
                    fallback_calls = self._fallback_description_calls(intent or {}, memory_scope)
                    fallback_ok, fallback_issue = self._calls_are_executable(fallback_calls, memory_scope, "replan")
                    if fallback_ok and fallback_calls:
                        calls = fallback_calls
                        state = "replan"
                        plan_repair = f"{initial_issue}；重规划失败后使用最小描述检索链"
                    else:
    ...
    def _latest_track_result_id(self, memory_scope: dict[str, Any]) -> str | None:
        for key in reversed(list(memory_scope.keys())):
            value = memory_scope.get(key)
            if isinstance(value, dict) and value.get("trackIds"):
                return str(key)
        return None

    def _latest_frame_result_id(self, memory_scope: dict[str, Any]) -> str | None:
        for key in reversed(list(memory_scope.keys())):
            value = memory_scope.get(key)
    ...
    def _fallback_description_calls(self, intent: dict[str, Any], memory_scope: dict[str, Any]) -> list[dict[str, Any]]:
        """描述类问题的最小可执行链，仅在模型计划不可用时启用。"""
        description = str(intent.get("description") or intent.get("targetText") or "").strip()
        if not description:
            return []
        track_id = self._latest_track_result_id(memory_scope)
        frame_id = self._latest_frame_result_id(memory_scope)
        ...
        if not track_id:
            calls.append({
                ...
                "tool": "getTrack",
                "arguments": {
                    "timeRange": intent.get("timeRange"),
                    "offset": 0,
                    "limit": int(intent.get("retrievalPageSize") or 60) or 60,
                },
            })
            track_ref = "tracks.trackIds"
        else:
            track_ref = f"{track_id}.trackIds"
        if not frame_id:
            calls.append({
                "id": "frames",
                "tool": "getFrames",
                "arguments": {"trackIds": {"$ref": track_ref}},
            })
            frame_ref = "frames.keyframes" if not track_id or "frames" in {c["id"] for c in calls} else f"{frame_id}.keyframes"
            if not track_id:
                frame_ref = "frames.keyframes"
            else:
                # 本轮新建 frames
                frame_ref = "frames.keyframes"
        else:
            frame_ref = f"{frame_id}.keyframes"
        calls.append({
            "id": "matchText",
            "tool": "matchText",
            "arguments": {
                "description": description,
                "galleryImages": {"$ref": frame_ref},
                "topK": 10,
            },
        })
        return self._auto_complete_calls(calls, memory_scope, intent)

    def _auto_complete_calls(
        self,
        calls: list[dict[str, Any]],
        memory_scope: dict[str, Any],
        intent: dict[str, Any],
    ) -> list[dict[str, Any]]:
        """补全 getTrack→getFrames→matchText 的常见依赖，不改写用户语义目标。"""
        if not calls:
            return []
        completed: list[dict[str, Any]] = []
        used_ids = set()
        track_source = self._latest_track_result_id(memory_scope)
        frame_source = self._latest_frame_result_id(memory_scope)
        ...
        def unique_id(base: str) -> str:
            candidate = base
            index = 1
            while candidate in used_ids or candidate in memory_scope:
                index += 1
                candidate = f"{base}{index}"
            used_ids.add(candidate)
            return candidate

        def ensure_track() -> str:
            nonlocal local_track_id, track_source
            if local_track_id:
                return local_track_id
            if track_source:
                return track_source
            call_id = unique_id("tracks")
            completed.insert(0, {
                "id": call_id,
                "tool": "getTrack",
                "arguments": {
                    "timeRange": intent.get("timeRange"),
                    "offset": 0,
                    "limit": int(intent.get("retrievalPageSize") or intent.get("maxRounds") and 60 or 60),
                },
            })
            # limit 修正：固定 60
            completed[0]["arguments"]["limit"] = 60
            local_track_id = call_id
            track_source = call_id
            return call_id

        def ensure_frames() -> str:
            nonlocal local_frame_id, frame_source
            if local_frame_id:
                return local_frame_id
            if frame_source:
                return frame_source
            track_id = ensure_track()
            call_id = unique_id("frames")
            completed.append({
                "id": call_id,
                "tool": "getFrames",
                "arguments": {"trackIds": {"$ref": f"{track_id}.trackIds"}},
            })
            local_frame_id = call_id
            frame_source = call_id
            return call_id

        for call in calls:
            tool = call.get("tool")
            call_id = str(call.get("id") or tool or "call")
            if call_id in used_ids:
                call_id = unique_id(call_id)
            else:
            ...
            if tool == "getTrack":
                ...
                if not arguments.get("limit"):
                    arguments["limit"] = 60
                if "timeRange" not in arguments:
                    arguments["timeRange"] = intent.get("timeRange")

            if tool == "getFrames":
                track_ids = arguments.get("trackIds")
                if not self._has_call_value(track_ids, used_ids | set(memory_scope), memory_scope):
                    source = ensure_track()
                    arguments["trackIds"] = {"$ref": f"{source}.trackIds"}
                local_frame_id = call_id
                frame_source = call_id
            ...
            if tool == "matchText":
                ...
                gallery = arguments.get("galleryImages")
                if not self._has_call_value(gallery, used_ids | set(memory_scope), memory_scope):
                    source = ensure_frames()
                    arguments["galleryImages"] = {"$ref": f"{source}.keyframes"}
                elif self._image_input_kind(gallery) == "unknown" and isinstance(gallery, dict) and "$ref" in gallery:
                    # 容错：把 tracks.xxx 误引用尽量改到 keyframes
                    ref = str(gallery.get("$ref") or "")
                    if ref.endswith(".trackIds") or ref.endswith(".tracks"):
                        source = ensure_frames()
                        arguments["galleryImages"] = {"$ref": f"{source}.keyframes"}

            if tool == "matchImage":
                # 缺一侧时不强行补全库图，避免错误语义；只补关键帧侧
                for field in ("queryImages", "galleryImages"):
                    value = arguments.get(field)
                    if not self._has_call_value(value, used_ids | set(memory_scope), memory_scope):
                        continue
                    if self._image_input_kind(value) == "unknown" and isinstance(value, dict) and "$ref" in value:
                        ref = str(value.get("$ref") or "")
                        if "keyframe" in ref.lower() or ref.endswith(".keyframes"):
                            source = ensure_frames()
                            arguments[field] = {"$ref": f"{source}.keyframes"}

            completed.append({"id": call_id, "tool": tool, "arguments": arguments, **({} if "condition" not in call else {"condition": call["condition"]})})
        return completed
    ...
    def _sanitize_value(self, value: Any) -> Any:
        ...
        if isinstance(value, str):
            ...
            reference = re.fullmatch(r"\s*\$ref\s*:\s*([\'\"]?)([^\'\"]+?)\1\s*", text_value)
            if reference:
                return {"$ref": reference.group(2).strip()}
            # 兼容 "$ref: tracks.trackIds" / "ref:tracks.trackIds" / 纯路径 tracks.trackIds
            loose = re.fullmatch(r"(?:\$ref|ref)\s*[:=]\s*([\'\"]?)([A-Za-z_][\w\-]*(?:\.[A-Za-z_][\w\-]*)+)\1", text_value)
            if loose:
                return {"$ref": loose.group(2).strip()}
            if re.fullmatch(r"[A-Za-z_][\w\-]*(?:\.[A-Za-z_][\w\-]*)+", text_value) and any(
                text_value.endswith(suffix)
                for suffix in (".trackIds", ".tracks", ".keyframes", ".keyframeIds", ".registryReferences", ".registryReferenceIds", ".matches")
            ):
